...
describe('telephony state', () => {
  it('returns null when no state exists', () => {
    expect(loadTelephonyState('/no/such/.offhook/telephony.json')).toBeNull();
  });

  it('writes (creating .offhook/) and reads back', () => {
    const dir = mkdtempSync(join(tmpdir(), 'offhook-tel-'));
    const path = join(dir, '.offhook', 'telephony.json');
    try {
